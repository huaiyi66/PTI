...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, attn_mask: Optional[torch.Tensor] = None, output_hidden_states=False):
        encoder_states = () if output_hidden_states else None

        for r in self.resblocks:
            x = r(x, attn_mask=attn_mask)
            if output_hidden_states:
                # encoder_states =  encoder_states + (x.detach().cpu(),)
                encoder_states =  encoder_states + (x.detach().cpu(),)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        if hasattr(self, 'output_hidden_states') and self.output_hidden_states:
            x, h = self.transformer(x, output_hidden_states=True)
            return x, h
        else:
            x = self.transformer(x) # torch.Size([1024, 1, 1664])
