...
nltk.data.path.append('/home/zhangcs/nltk_data/nltk_data/nltk_data')
import pdb
...
class CHAIR(object):
    ...
    def compute_chair(self, cap_file, image_id_key, caption_key):
        ...
        # print(num_hallucinated_caps, num_caps)
        # print(hallucinated_word_count, coco_word_count, chair_i)

        if num_hallucinated_caps > 0:
            hps = (hallucinated_word_count / num_hallucinated_caps)
        else:
            hps = 0.0  # 如果没有句子产生幻觉，则严重性为0
        ...
        object_freq = num_generated_objects / len_caps
...
def check_file_line_count(cap_file, expected_count=500):
    """
    Check if the given JSONL file has the expected number of lines.
    """
...
if __name__ == '__main__':
    ...
    parser.add_argument("--coco_path", type=str, default='../download_datasets/annotations',
    ...
    # # Check if the file has the expected number of lines (500)
    # if not check_file_line_count(args.cap_file, expected_count=500):
    #     print(f"Error: The file {args.cap_file} does not contain 500 lines. Evaluation will be skipped.")
    #     sys.exit(1)

    # pdb.set_trace()
    # evaluator = pickle.load(open('chair_train.pkl', 'rb'))
    # evaluator.imid_to_objects
    # len( evaluator.imid_to_objects.keys())
    #  evaluator.imid_to_objects[157176]
